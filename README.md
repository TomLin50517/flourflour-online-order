...
專案本身是一般的 Next.js Server（Node.js runtime），部署到 Vercel 或任何一般 Node.js 主機（Railway/Render/Fly.io 等）沒有已知障礙。若考慮 **Cloudflare Workers**，目前至少有三個已知的相容性問題需要處理，並非開箱即用：

1. `bcrypt`（後台登入密碼比對）是原生 C++ binding，Workers runtime 不支援，需改用純 JS 的 `bcryptjs`。
2. Prisma 目前用 `@prisma/adapter-pg`（node-postgres，走原生 TCP 連線），Workers runtime 預設不能開 TCP 連線到資料庫，需改用 Cloudflare Hyperdrive，或換成提供 HTTP driver 的 Postgres 代管服務（如 Neon）。
3. `src/proxy.ts`（Next.js Middleware）目前會被判定為需要 Node.js runtime（因為間接引用了 `bcrypt`／Prisma），Cloudflare 的 `@opennextjs/cloudflare` adapter 目前不支援 Node.js Middleware，需要把 middleware 裡會碰到這些相依的邏輯拆到 Node.js runtime 的路由層，middleware 本身只保留 edge-safe 的部分（語系協商、session cookie 存在與否的判斷）。

資料庫本身維持 Postgres 即可（`SPEC.md` ADR-2 的選型理由——訂單狀態機的樂觀鎖、取貨號原子遞增都需要 Postgres 等級的交易一致性），不建議為了配合 Workers 換成 SQLite/D1；改連線方式（Hyperdrive 或 Neon 之類的 serverless Postgres）即可，schema 不需要變動。
