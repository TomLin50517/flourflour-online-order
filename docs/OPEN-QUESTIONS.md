...
### 正式站圖片上傳 500：`@cf-wasm/photon` 的 workerd 進入點在 Next.js bundler 下建置期就失敗（未解決）
...
- 暫定假設：**先撤回所有嘗試，`src/lib/image-processing.ts` 維持 `/node` 進入點**，讓 build/deploy 恢復正常（本機開發／測試不受影響，`/node` 本來就沒問題）。正式站的圖片上傳功能目前**確定會 500**，這是新發現、獨立於 Drizzle 遷移之外的既有 bug，R2 儲存本身的設定（bucket、公開網址、`STORAGE_*`／`NEXTAUTH_*` 等 secrets、`next.config.ts` 的 `images.remotePatterns`／CSP `img-src`）都已確認正確、就緒；只差圖片重編碼這一步在正式站上還無法執行。
- 已知但尚未嘗試、供下次接手參考的方向：(a) 官方 README 有專門的「Next.js App Router + Edge Runtime」範例，用 `export const runtime = "edge"` + `@cf-wasm/photon/edge-light`——Next.js 的 edge runtime 是獨立於一般 Node.js server 的 bundling 目標，理論上對 wasm 的處理方式不同，值得驗證是否真的可行，但要注意 `/api/v1/admin/uploads` 這支 route 同時用了 `node:crypto`（`randomUUID`）、`requireAdmin()`（NextAuth session）、`@aws-sdk/client-s3`，改成 edge runtime 前要先確認這些依賴在 edge runtime 下是否都能正常運作；(b) 改用 Cloudflare 原生的 Image Resizing / Images binding（`env.IMAGES`，較新版 Wrangler 支援），完全不依賴任何 npm 端的 wasm 套件，這是跟「Prisma → Drizzle」同等級的治本方向（換掉有問題的依賴，而非跟 bundler 纏鬥），但需要另外評估功能是否涵蓋目前 `reencodeToWebp()` 做的 EXIF 校正／resize／webp 轉檔需求；(c) 回報 `@cf-wasm/photon` 或 Next.js 上游 issue。
- 影響範圍：`src/lib/image-processing.ts`（維持 `/node`，未變更）；`next.config.ts`（曾短暫加過 `webpack()` 設定與 `serverExternalPackages` 調整，均已還原）；`/api/v1/admin/uploads` 在此問題解決前於正式站持續回應 500；R2 相關設定（bucket `flourflour`、`wrangler secret put` 設定的 `STORAGE_*`/`NEXTAUTH_*`/`APP_BASE_URL`/`PAYMENT_PROVIDER`/`MOCK_WEBHOOK_SECRET`、`.env.production` 的 `STORAGE_PUBLIC_BASE_URL`）皆已就緒且正確，不受此問題影響。
