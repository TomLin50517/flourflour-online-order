...
### Hyperdrive query cache 造成短暫的讀寫不一致（未決定是否關閉）
...
- 影響範圍：`src/app/(admin)/admin/(dashboard)/orders/page.tsx`（已修正）；`src/app/(admin)/admin/(dashboard)/products/ProductForm.tsx`（受影響、未修正——目的地是 Server Component 頁面，無法比照訂單看板的修法）；Hyperdrive 資源 `flourflour-db`（`c3e4cf1358434f15a3a425b85826c342`）目前快取設定維持預設啟用，未變更。
